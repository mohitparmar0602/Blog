...
def register_routes(app):
    ...
    def _sync_tags(post: Post, tags_raw: str) -> None:
        ...
        if not tags_raw:
            return
        names = {t.strip().lower() for t in tags_raw.split(",") if t.strip()}
        for name in names:
    ...
    def login():
        ...
        form = LoginForm()
        if form.validate_on_submit():
            recaptcha_token = request.form.get(
    ...
    def change_role(user_id):
        ...
        flash(
            f"{user.username}'s role changed to {new_role}.",
            "success",
        )
    ...
    def delete_user(user_id):
        ...
        flash(
            f"User {user.username!r} deleted.",
            "info",
        )
        return redirect(url_for("dashboard"))

    @app.route(
        "/admin/post/<int:post_id>/toggle",
        methods=["POST"],
    )
    @login_required
    @admin_required
